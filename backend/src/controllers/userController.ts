...
export const getAllUsers = asyncHandler(async (_req: AuthRequest, res: Response<ApiResponse>) => {
...
export const approveHelper = asyncHandler(async (req: AuthRequest, res: Response<ApiResponse>) => {
  if (!req.user || req.user.role !== 'admin') {
    res.status(403).json({
      success: false,
      error: 'Only admins can approve helpers'
    });
    return;
  }

  const userId = parseInt(req.params.id);
  const user = await userService.approveHelper(userId);
